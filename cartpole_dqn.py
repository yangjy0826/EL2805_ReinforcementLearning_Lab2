...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                # if e>=EPISODES-100:
                #     print('average scaore over last 100 episode:', np.mean(scores[-min(100, len(scores)):]))
    ...
    pylab.figure()
    pylab.plot(episodes[-min(100, len(scores)):], scores[-min(100, len(scores)):], 'b')
    pylab.xlabel("Episodes")
    pylab.ylabel("Average Score in last 100 episode")
    pylab.savefig("AverageScoresLast100Episode.png")
